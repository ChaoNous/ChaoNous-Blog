...
function errorResponse(code: CommentApiErrorCode, message: string, status: number, headers?: HeadersInit): Response {
    return json({
        ...
        code,
        message,
    } satisfies ApiErrorPayload, status, headers);
}
...
export function paginateNestedComments(records: NormalizedComment[], page: number, limit: number): {
    data: NestedComment[];
    totalCount: number;
} {
...
export function createPagination(page: number, limit: number, totalCount: number): PaginationMeta {
    return {
        page,
        limit,
...
export function unauthorized(message: string = COMMENT_MESSAGES.unauthorized): Response {
    return errorResponse("UNAUTHORIZED", message, 401);
}
export function tooManyRequests(message: string, retryAfterSeconds = 60): Response {
    return errorResponse("TOO_MANY_REQUESTS", message, 429, {
        "retry-after": String(retryAfterSeconds),
    });
}
export function ensureSameOrigin(request: Request, message: string = COMMENT_MESSAGES.invalidOrigin): Response | null {
...
export function notFound(message: string = COMMENT_MESSAGES.notFound): Response {
    return errorResponse("NOT_FOUND", message, 404);
}
export function serverError(message: string = COMMENT_MESSAGES.serverError): Response {
        ...
        switch (contentPolicy.reason) {
            case "submitted_too_fast":
                return {
                    ...
                    retryAfterSeconds: Math.ceil(COMMENT_SUBMISSION_POLICY.minFormFillMs / 1000),
                } as const;
            case "too_many_links":
                return {
                    ok: false,
...
export async function enforceSubmissionRateLimit(env: Env, input: {
    postSlug: string;
    email: string;
    content: string;
    now?: number;
    ...
    const [postWindowRow, globalWindowRow, duplicateRow] = await Promise.all([
        env.COMMENTS_DB.prepare(`SELECT COUNT(*) AS recent_count, MAX(created_at) AS last_created_at
       FROM comments
       WHERE post_slug = ?1
         AND author_email = ?2
         AND created_at >= ?3`)
            .bind(input.postSlug, input.email, now - COMMENT_SUBMISSION_POLICY.recentPostWindowMs)
            .first<{
            recent_count: number;
            last_created_at: number | null;
        }>(),
        env.COMMENTS_DB.prepare(`SELECT COUNT(*) AS recent_count
       FROM comments
       WHERE author_email = ?1
         AND created_at >= ?2`)
            .bind(input.email, now - COMMENT_SUBMISSION_POLICY.recentGlobalWindowMs)
            .first<{
            recent_count: number;
        }>(),
        env.COMMENTS_DB.prepare(`SELECT MAX(created_at) AS last_created_at
       FROM comments
       WHERE post_slug = ?1
         AND author_email = ?2
         AND content = ?3
         AND created_at >= ?4`)
            .bind(input.postSlug, input.email, input.content, now - COMMENT_SUBMISSION_POLICY.duplicateWindowMs)
            .first<{
            last_created_at: number | null;
        }>(),
    ]);
    ...
    return {
        ok: false,
        message: decision.reason === "duplicate_content"
            ? COMMENT_MESSAGES.duplicateComment
            : COMMENT_MESSAGES.commentRateLimited,
        retryAfterSeconds: decision.reason === "duplicate_content"
...
export async function deleteCommentsByIds(env: Env, ids: number[]): Promise<number> {
  ...
  `;
    const countRow = await env.COMMENTS_DB.prepare(`${subtreeSql}
    ...
    if (!totalCount)
        return 0;
    await env.COMMENTS_DB.prepare(`${subtreeSql}
