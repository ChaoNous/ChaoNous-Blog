...
function loadEcharts(): Promise<typeof window.echarts> {
    ...
    window.__btcAnalysisEchartsPromise = new Promise((resolve, reject) => {
        const existingScript = document.querySelector<HTMLScriptElement>(`script[src="${ECHARTS_CDN_URL}"]`);
        if (existingScript) {
            existingScript.addEventListener("load", () => resolve(window.echarts), { once: true });
            existingScript.addEventListener("error", () => reject(new Error("Failed to load ECharts")), { once: true });
            return;
        }
...
function calcMovingAverage(data: number[], period: number): (number | null)[] {
...
function renderForecastTable(forecast: BtcForecastEntry[], tableEl: HTMLElement): void {
    const fmt = (value: number): string => `$${Math.round(value).toLocaleString()}`;
    let html = '<table style="width:100%;border-collapse:collapse;font-size:0.82rem"><thead><tr style="border-bottom:1px solid rgba(128,128,128,0.15)"><th style="padding:8px 12px;text-align:left;font-size:0.72rem;color:#5a7a9a;text-transform:uppercase;letter-spacing:0.04em">日期</th><th style="padding:8px 12px;text-align:right;font-size:0.72rem;color:#5a7a9a">预测价格</th><th style="padding:8px 12px;text-align:right;font-size:0.72rem;color:#5a7a9a">68% 区间</th><th style="padding:8px 12px;text-align:right;font-size:0.72rem;color:#5a7a9a">95% 区间</th></tr></thead><tbody>';
    for (let index = 0; index < forecast.length; index += 1) {
        const row = forecast[index];
        const week = Math.ceil((index + 1) / 7);
        const weekMark = (index + 1) % 7 === 0
